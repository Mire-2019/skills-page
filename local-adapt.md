...
| 2 | **重写/融合 SKILL.md** | 全中文正文 + 本地约束（D 盘/fid/口径/CDP/file:// 交付/证据分层）+ `source:` + `version: 1.1.0` + 「关于本文件」 |
...
- **全中文**：正文/注释中文，术语（fid/pathId/CDP/cube/measure）除外。
- **落盘 D 盘**：一切产物 D 盘；脚本禁 C 盘路径。
- **交付 file://**：HTML 交付一律本地绝对路径开；上传妙搭/外发先审批。
- **证据分层**：事实 / 推断 / 待确认，不空口断言；数字带口径引用。
- **工具范式**：项目自有工具优先（如 Quick BI 走 CDP 复用登录、飞书走 lark-cli 身份分清楚、报告走正规工作流），通用工具范式可提；报告走 qbi-html 正规工作流。
